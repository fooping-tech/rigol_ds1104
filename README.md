...
```text
RIGOL TECHNOLOGIES,DS1104Z Plus,DS1ZC243504233,00.04.05.SP2
```

`192.168.3.81` では次の両方で `*IDN?` が成功しました。

```text
TCPIP0::192.168.3.81::INSTR
TCPIP0::192.168.3.81::5555::SOCKET
```
...
```bash
uv run python tools/rigol/rigol_check_lan.py --ip 192.168.3.81
uv run python tools/rigol/rigol_check_lan.py --ip 192.168.3.81 --socket
```

スクリーンショットを保存します。

```bash
uv run python tools/rigol/rigol_screenshot.py --ip 192.168.3.81 --outdir captures
```
...
```bash
uv run python tools/rigol/rigol_screenshot.py --ip 192.168.3.81
```
...
```bash
uv run python tools/rigol/rigol_single_capture.py --ip 192.168.3.81 --channel CHAN1 --slope falling
```
...
```bash
uv run python tools/rigol/rigol_waveform_csv.py --ip 192.168.3.81 --channel CHAN1
```
...
このリポジトリを GitHub で公開する場合は、次の構成を保つと利用者が導入しやすくなります。
...
Claude Code project skill として最初から同梱したい場合は、同じ Skill を `.claude/skills/rigol-ds1104z-lan/` にも置けます。ただし、`.codex/skills/` と `.claude/skills/` の内容がずれないように管理してください。重複管理を避けたい場合は README のコピー手順だけを案内する運用で十分です。
...
実機通信は `192.168.3.81` に対して `INSTR` と socket mode の両方で成功済みです。制限された実行環境では ICMP `ping` が失敗しても、SCPI 通信は成功する場合があります。
